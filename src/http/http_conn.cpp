...
http_conn::HTTP_CODE http_conn::do_get_request()
{
    ...
    address = (char *)mmap(nullptr, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
    close(fd);
...
bool http_conn::write()
{
    ...
    while (true)
    {
        temp = (int)writev(m_sockfd, m_iv, m_iv_count);
